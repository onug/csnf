    ...
    try:
        with open(input_file) as f:
            ...
            for (
                ...
                alert_mapping = provider["source"][source_name]["alerts"][
                    alert_id_name
                ]["alertMapping"]
...
def dict_to_json(all_providers, output_file):
    try:
        with open(output_file, "w") as outfile:
            json.dump(all_providers, outfile)
    except Exception as e:
...
def dict_to_splunk_conf(all_providers):
    try:
        ...
        with open("./props.conf", "w") as outfile:
            for provider, provider_values in all_providers.items():
                for source, source_values in provider_values["source"].items():
                    ...
                    for alertMapping, alertMapping_values in source_values["alerts"][
                        "__default__"
                    ]["alertMapping"].items():
                        alias_prefix = "FIELDALIAS-csnf-"
                        prefix = alias_prefix + alertMapping.replace(".", "-")
                        print(
                            prefix
                            + " = "
                            + '"'
                            + alertMapping_values["path"]
                            + '"'
                            + " ASNEW "
                            + '"csnf.'
                            + alertMapping
        ...
        raise "Failed to write Splunk props.conf file"
...
def set_parser_arguments():
    ...
    parser.add_argument(
        "-o", type=argparse.FileType("r"), dest="input.csv", help="Input CSV File"
    )
    parser.add_argument(
        "-i", type=argparse.FileType("w"), dest="output_json", help="JSON Output prefix"
    )
...
def execute_conversion():
    ...
    all_providers_dict = convert_onug_csv_to_json(
        result.input_csv.name, result.output_json.name
    )
    dict_to_json(all_providers_dict, result.output_json.name)
