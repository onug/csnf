...
class Onug:
    ...
    def __set_source_regex_provider_mapping(self):
        for provider_key, provider_value in self.__onug_mapping.items():
            for source_key, source_value in provider_value["source"].items():
                self.__source_to_provider_mapping[
                    source_value["sourceIdentifierRegex"]
                ] = {"provider": provider_key, "source": source_key}
                logging.debug(
                    "__set_source_regex_provider_mapping: Added this to mapping"
                    + str(
                        self.__source_to_provider_mapping[
                            source_value["sourceIdentifierRegex"]
                        ]
    ...
    def __set_provider_source_raw_finding(self):
        ...
        logging.error(
            "__set_provider_source_raw_finding: FAILED TO GET SOURCE AND PROVIDER"
        )
        raise SystemExit(
            "__set_provider_source_raw_finding: FAILED TO GET SOURCE AND PROVIDER"
        )
    ...
    def __set_finding_provider_data(self):
        ...
        self.__finding["provider"]["providerId"] = self.__onug_mapping[self.__provider][
            "providerId"
        ]
        logging.debug(
            "__set_provider_from_finding: ProviderId is set to: "
            + self.__finding["provider"]["providerId"]
        )
        self.__finding["provider"]["providerType"] = self.__onug_mapping[
            self.__provider
        ]["providerType"]
        logging.debug(
            "__set_provider_from_finding: ProviderId is set to: "
            + self.__finding["provider"]["providerType"]
        )
        self.__finding["provider"]["name"] = self.__onug_mapping[self.__provider][
            "provider"
        ]
        logging.debug(
            "__set_provider_from_finding: Provider is set to: "
            + self.__finding["provider"]["name"]
        )
        return
    ...
    def __set_onug_provider_mapping(self):
        ...
        try:
            ...
            logging.error(
                "__set_onug_provider_mapping: FAILED TO GET REQUEST" + str(err)
            )
    ...
    def __set_onug_mapping_for_source(self):
        ...
        try:
            self.__source_mapping = self.__onug_mapping[self.__provider]["source"][
                self.__source
            ]
        except Exception as err:
            logging.error(
                "__set_onug_mapping_for_source: FAILED Provider from Provider JSON"
                + str(err)
            )
            raise SystemExit(err)
    ...
    def __set_item_from_path(self, path, item):
        # Cheating
        logging.debug(
            "__set_item_from_path: path is: " + str(path) + " Item is: " + str(item)
        )
        keys = path.split(".")
        ...
        for alert_key, alert_value in self.__source_mapping["alerts"].items():
            for map_key, map_value in alert_value["alertMapping"].items():
                for key, value in map_value.items():
                    logging.debug(
                        "__map_raw_finding_to_onug: Mapped Value: " + str(map_key)
                    )
                    if not (map_value["mappedValue"]):
                        path = map_value["path"]
                        mapped_item = self.__mapped_item_from_path(
                            path, self.__raw_finding
                        )
